...
export const updateDebtNotes = async (req, res) => {
  try {
    ...
    // Update contract if contract data is present
    if (isContractUpdate) {
      const {
        interestEnabled,
        interestEveryDays,
      ...
      // If both interest and late fee are disabled, remove the contract entirely
      if (!interestEnabled && !lateFeeEnabled) {
        note.contract = null;
      } else {
        note.contract = {
          interest: {
            enabled: interestEnabled || false,
            everyDays: interestEnabled ? (interestEveryDays || 30) : null,
            chargeAmount: interestEnabled ? (interestChargeAmount || 0) : null,
          },
          lateFee: {
            enabled: lateFeeEnabled || false,
            everyDays: lateFeeEnabled ? (lateFeeEveryDays || 7) : null,
            chargeAmount: lateFeeEnabled ? (lateFeeChargeAmount || 0) : null,
          },
