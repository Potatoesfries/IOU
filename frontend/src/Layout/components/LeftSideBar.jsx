...
import { useDebtNotes } from "../../context/DebtNotesContext.jsx";
...
const LeftSideBar = () => {
  const { user } = useUser();
  const { debtNotes, loading, fetchDebtNotes } = useDebtNotes();
...
// Enhanced search function with sorting
  const filteredDebtNotes = useMemo(() => {
    let filtered = debtNotes;

    // Apply search filter if search term exists
    if (searchTerm.trim()) {
      const search = searchTerm.toLowerCase().trim();
      filtered = debtNotes.filter(debt => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      // Fallback: Use _id (MongoDB ObjectIds are sortable by creation time)
      if (a._id && b._id) {
        return b._id.localeCompare(a._id);
      }
      
      // Final fallback: sort by due date (most recent first)
